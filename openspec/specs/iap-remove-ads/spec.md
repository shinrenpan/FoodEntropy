# iap-remove-ads Specification

## Purpose

The app's only paid path: a one-time non-consumable that removes the banner `advertising` shows. Its correctness rests on one rule — ownership is derived from StoreKit entitlements on every check and never cached locally. Adding a local purchased flag would look like an optimisation and would reintroduce both problems this design removes: refunds and revocations never write back to a local flag (so a refunded user keeps the benefit), and anything written to user defaults can be edited to unlock for free. The restore button is technically redundant under this design and is kept because Apple's review expects one on a non-consumable.
...
<!-- @trace
source: baseline-iap-remove-ads
updated: 2026-08-08
code:
