...
The single user-facing surface for three domain capabilities — `iap-remove-ads`, `icloud-sync`, and `notification` — none of which the user can otherwise reach. It owns no data of its own; every value is loaded fresh each time the screen appears, because all of them can change while it is off screen (permission granted in system Settings, a purchase made on another device, a refund clearing). The notification row's split matters: an undecided user gets the system prompt directly, while a decided one is sent to system Settings, because the in-app prompt never appears twice and calling it again would look like a dead tap.
