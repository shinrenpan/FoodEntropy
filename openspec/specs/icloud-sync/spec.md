...
A single boolean preference read once at launch — the actual syncing is entirely the CloudKit-backed container's job. Three of its properties look like unfinished work and are deliberate: it defaults to off (so turning it on *is* the consent, which is what lets the app ship with no onboarding), it applies only at the next launch (swapping a live `ModelContainer` is the one operation most likely to lose data, and "restart the app" always succeeds), and turning it off leaves the cloud copy intact (off means stop syncing, not delete my backup). This design only works because `persistence` keeps the schema CloudKit-safe at all times, so neither direction needs a migration.
...
<!-- @trace
source: baseline-icloud-sync
updated: 2026-08-08
code:
  - Sources/Core/AppPreferences.swift
