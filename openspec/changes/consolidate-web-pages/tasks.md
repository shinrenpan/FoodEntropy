...
- [ ] 2.5a 刪除本 repo 的 `github-pages` environment 與部署記錄（於 2.2 關閉 Pages 之後）；驗證：`gh api repos/shinrenpan/FoodEntropy/environments` 的 `total_count` 為 0，`deployments` 為空陣列。此 environment 建於 2026-07-23，與 `docs/` 同日產生，是啟用 Project Pages 的副產物。
