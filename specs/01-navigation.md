...
### Row 操作（4 種出口，詳見 `02-architecture` §2.3；操作面向見 `03-screens/home.md`）
- **向右滑（leading）**：標記已使用（`consumed`，移出清單）
- **向左滑（trailing）**：刪除（hard delete，**跳確認**）
- **長按 → context menu**：延長效期 / 標記已使用 / 標記丟棄（`wasted`）——只放滑動 / 點擊之外的操作，避免重複
- **單點整列 row** → push 進編輯頁（可改名稱 / 購買日 / 到期日 / 圖片，見 §4）
- **發現性**：清單非空時 Section footer 顯示 hint 告知可長按（issue #2）。
